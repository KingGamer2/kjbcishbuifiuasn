...
client.oN("message", message => {
        if (message.content === adminprefix + "setprefix") {
...
.setDescription('👑أوامر الأعضاء👑')
      ...
      .addField('❖-|!server', `➰يجبلك معلومات عن السيرفر➰`)
	   .addField('❖-|!updat', `👑حتى تشوف اخر تحديثات للبوت💯`)
...
client.on("message", message => {
 if (message.content === prefix + "update") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setDescription('🎆ما الجديد؟🎆')
          .addField('❖-|!new', `👑حتى تسوي تكت💯`)
	  .addField('❖-|!clr', `❌لخاصية حذف شات بعدد محدد❎`)
	  	  .addField('❖-|!giveaway', `👑امر عمل قيف اوي🎆`)
	  .addField('❖-|.', `❌تم حذف لعبة صراحة و كت تويت❎`)
	  .addField('❖-|.', `👑#credits و !credits تم الغاء ميزت لكردت  لطلب الكثير لان في ناس غلطت بين 🔰`)
 	  .addField('❖-|.', `🔰تمت اضافة منع لسبام و السب🎆`)
	  .addField('❖-|.', `👑تم اصلاح بعد الاخطاء🎆`)
  message.author.send({embed});
      message.channel.send(":white_check_mark: | Check Your DM تم الأرسال بلخاص")
 }
});
